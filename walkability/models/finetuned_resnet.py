...
# SATELLITE IMAGERY-TRAINED RESNET18:
class Satellite_Resnet(pl.LightningModule):
    def __init__(self, num_classes=3, learning_rate=1e-5):
        super().__init__()
        self.save_hyperparameters()
        # load ResNet18 pretrained on satellite imagery (EuroSAT/Sentinel-2)
        self.model = tg_models.resnet18(weights=tg_models.ResNet18_Weights.SENTINEL2_ALL_MOCO)
    ...
    # training / validation / test step functions are the same
    def training_step(self, batch, batch_idx): 
        '''
        What happens in one step of the training loop (calculating loss, etc.)
        '''
